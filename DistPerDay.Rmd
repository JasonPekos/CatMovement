...
age_distinct <- (ref
        %>% dplyr::select(animal.life.stage)
         %>% filter(!is.na(animal.life.stage), animal.life.stage!=0)
        %>% distinct()
        %>% arrange(animal.life.stage)
  
)


#TO DO combine day night, to see if changes in sex
# see if I can add age to this plot somehow
# distance by latitude
# speed of cat over time
```
...
# + geom_jitter(aes(alpha = 0.3, shape = animal.reproductive.condition),width=0.3, height=0)
 + geom_jitter(aes(alpha = 0.3),width=0.3, height=0)
...
# Questions
#1. What smoothing should be used?
...
(ggplot(avg_day_night, aes(x=trackId,y=avg, colur=diurnal))
 #+ geom_boxplot()
...
(ggplot(avg_day_night, aes(x=diurnal,y=avg, colur=diurnal))
 + geom_boxplot()
+ scale_y_log10()
 )

#age
(ggplot(total, aes(x=seq(1,1660,by=1),y=avg, colour=animal.life.stage))
 #+ geom_boxplot()
 + geom_point(aes(colour=animal.life.stage, size=4))
 #+ geom_smooth()
 + scale_y_log10()
  + facet_grid(rows = vars(diurnal))
  + scale_color_gradient(low = "white", high = "red")
  + theme_bw()
 )

#sex, facet by day/night
(ggplot(total, aes(x=seq(1,1660,by=1),y=avg, colour=animal.sex))
 #+ geom_boxplot()
 + geom_point(aes(colour=animal.sex, alpha = 0.4,size=4))
 + geom_smooth()
 + scale_y_log10()
  + facet_grid(cols = vars(diurnal))
  #+ scale_color_gradient(low = "yellow", high = "red")
...
#sex
(ggplot(total, aes(x=seq(1,1660,by=1),y=avg, colour=sex_diurnal))
 #+ geom_boxplot()
 + geom_point(aes(colour=sex_diurnal, alpha = 0.4,size=4))
 + geom_smooth()
 + scale_y_log10()
  #+ facet_grid(rows = vars(diurnal))
  #+ scale_color_gradient(low = "yellow", high = "red")
...
(ggplot(total, aes(x=animal.sex,y=avg, colur=animal.sex))
 + geom_boxplot()
+ scale_y_log10()
 )
...
1. Data Cleaning
  ...
  * Removed cats with values that broke the dBBM model / took 5min to compute for no recognizable reason. 
...
* Are their cycles or patterns to the distance travelled? 
