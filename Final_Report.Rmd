...
```{r Plot_1,echo=FALSE, message=FALSE,warning=FALSE,fig.height=5,fig.width=7}
FullPlot = ggplot(cats_diff, aes(x = timestamp, y = Country_name, colour = Country,
  ...
  geom_line() +     
  theme_bw() +
  # geom_rect(ymin = which(cats_diff$Country=="Australia"), ymax = Inf,
  #           xmin = -Inf, xmax = Inf, fill="red") +
...
subplot(ggplotly(FullPlot,tooltip=c("text"),showlegend=TRUE),DensityPlot,
        nrows=2,heights=c(0.7, 0.2),shareX=TRUE)

#tooltips for density need to be fixed
#where is legend?
```
